...
Rule 1：所有可变参数放入YAML配置文件，配置变更热重载，禁止代码硬编码
Rule 2：多模型后端兼容（OpenRouter / 阿里云百炼 / OpenAI）
Rule 3：提示词文件化，system/user/examples分离，string.Template变量渲染

职责：
- YAML配置加载与热重载（检查文件mtime）
- 多模型后端配置解析（backend: openrouter|dashscope|openai）
- API Key从环境变量读取，禁止硬编码
- 提示词文件（system.txt/user.txt/examples.txt）读取与拼接
- 提示词模板变量渲染（string.Template.safe_substitute）
"""
...
class ConfigManager:
    ...
    支持：
    - YAML配置热重载（文件mtime变更自动重载）
    - 多后端LLM配置（openrouter / dashscope / openai）
    - API Key优先从环境变量读取
    - 提示词文件路径配置与动态加载
    - string.Template变量渲染

    Attributes:
        config_path: YAML配置文件路径（相对于工作目录）
        config_data: 解析后的完整配置字典
        _file_mtimes: 各配置文件的最后修改时间（用于热重载检测）
    """

    def __init__(self, config_path: str = "prompts/standard.yaml") -> None:
        """
        初始化配置管理器并立即加载配置
    ...
    def _load_config(self) -> None:
        """
        加载YAML主配置文件，并记录所有相关文件的mtime

        同时预读取提示词文件的mtime，以便热重载时检测变化。
        """
    ...
    def load_prompts(self) -> Tuple[str, str]:
        ...
        加载逻辑：
        1. 从standard.yaml读取 prompts.system_file / user_file / examples_file
        2. 读取 system.txt 内容
        3. 读取 user.txt 内容
    ...
    def render_prompts(
        ...
        **extra_vars: Any,
        ...
        - $hierarchy_path：层级路径
        - $bg_color / $text_color / $font：来自style配置
        - $primary / $secondary / $accent / $bg_light 等：来自style.colors配置
        ...
        # 构建模板变量字典（扁平化colors子键，方便$primary等直接引用）
        ...
        # 使用 string.Template.safe_substitute 渲染（未定义变量保持原样）
        rendered_system = Template(system_template).safe_substitute(template_vars)
        rendered_user = Template(user_template).safe_substitute(template_vars)
